...
scenarios_barplot <- function(dt.list, 
  ...
  x_values <- barplot(height = rep(0, length(sectors)), 
                      space = length(dt.list), col = 0, ylim = c(0, max(unlist(lapply(dt.list, "[", ,2))) + ylim.adjust), 
                      las = 2)
  
  if(grid) abline(h = seq(0, signif(max(unlist(lapply(list, "[", ,2))), 2), xaxt.steps), col = "grey30", lty = 2, lwd = 0.5)
  ...
  mtext(side = 1, text = unlist(dt.list[[1]][,1]), at = x_values+0.5, las = 2, line = 1, cex = 0.7)
  if(legend) legend(legend.pos, legend = names(dt.list), fill = colorvec,box.col = "white", bg = "white" )
}
